...
class WordSearchPuzzle {
  ...
  PlacedWord? wordForSelection(List<GridPoint> selection) {
    for (final word in words) {
      ...
      if (_sameCells(selection, cells) || _sameCells(selection, cells.reversed)) {
        return word;
      }
    }
    return null;
  ...
  static List<GridPoint>? lineBetween(GridPoint start, GridPoint end) {
    ...
    return [
      for (var index = 0; index < length; index++)
        GridPoint(start.row + rowStep * index, start.column + columnStep * index),
    ];
  }
...
class WordSearchGenerator {
  ...
  WordSearchPuzzle generate({
    ...
    if (fillerCharacters.isEmpty) {
      throw ArgumentError.value(fillerCharacters, 'fillerCharacters', 'Cannot be empty.');
    }
    final unique = <String>{};
    final words = <String>[];
    ...
    for (final word in words) {
      ...
      for (var index = 0; index < letters.length; index++) {
        cells[placement.start.row + placement.direction.rowStep * index]
            [placement.start.column + placement.direction.columnStep * index] = letters[index];
      }
    ...
    for (var row = 0; row < size; row++) {
      for (var column = 0; column < size; column++) {
        cells[row][column] ??= fillerCharacters[_random.nextInt(fillerCharacters.length)];
      }
    }
  ...
  PlacedWord? _findPlacement(List<List<String?>> cells, String word) {
    final starts = [
      for (var row = 0; row < cells.length; row++)
        for (var column = 0; column < cells.length; column++) GridPoint(row, column),
    ]..shuffle(_random);
    ...
    for (final direction in directions) {
      for (final start in starts) {
        ...
        final endColumn = start.column + direction.columnStep * (letters.length - 1);
        if (endRow < 0 || endColumn < 0 || endRow >= cells.length || endColumn >= cells.length) {
          continue;
        }
        var canPlace = true;
        for (var index = 0; index < letters.length; index++) {
          final current = cells[start.row + direction.rowStep * index]
              [start.column + direction.columnStep * index];
